...
const HEADERS = ["업무ID", "작성자이메일", "구분", "업무명", "마감일", "우선순위", "메모", "완료여부", "자동생성", "상위업무ID", "기준행사명", "기준Dday"];
...
function getTasks() {
  return getMyTasks().map(function (task) {
    return {
      ...
      workflowOffset: task.workflowOffset
    };
  });
}
...
function createTask(input) {
  const created = addNewPersonalTask({
    ...
    autoGenerated: input && input.autoGenerated,
    parentTaskId: input && input.parentTaskId,
    workflowTitle: input && input.workflowTitle,
    workflowOffset: input && input.workflowOffset
  });
  return {
    id: created.id,
    ...
    owner: created.owner,
    autoGenerated: created.autoGenerated,
    parentTaskId: created.parentTaskId,
    workflowTitle: created.workflowTitle,
    workflowOffset: created.workflowOffset
  };
}
...
function validateTask_(input) {
  ...
  return {
    ...
    autoGenerated: input.autoGenerated === true || String(input.autoGenerated).toLowerCase() === "true",
    parentTaskId: String(input.parentTaskId || "").slice(0, 100),
    workflowTitle: String(input.workflowTitle || "").trim().slice(0, 100),
    workflowOffset: Math.max(0, Number(input.workflowOffset || 0))
  };
}
...
function rowToLegacyTask_(row) {
  return {
    ...
    autoGenerated: row[8] === true || String(row[8]).toLowerCase() === "true",
    parentTaskId: String(row[9] || ""),
    workflowTitle: String(row[10] || ""),
    workflowOffset: Number(row[11] || 0)
  };
}
