...
function taskMatchesCurrentView(task) {
  ...
  const matchesQuery = !keyword || `${task.title} ${task.memo} ${task.category}`.toLocaleLowerCase("ko").includes(keyword);
...
function workflowGroups() {
  ...
  state.tasks.filter((task) => task.autoGenerated).forEach((task) => {
    const key = task.parentTaskId || `orphan-${task.workflowTitle}`;
    if (!childMap.has(key)) childMap.set(key, []);
    childMap.get(key).push(task);
  });

  const groups = state.tasks.filter((task) => !task.autoGenerated).map((parent) => ({
    id: parent.id,
    parent,
    steps: (childMap.get(parent.id) || []).sort((a, b) => b.workflowOffset - a.workflowOffset)
  }));

  const parentIds = new Set(groups.map((group) => group.parent.id));
  childMap.forEach((steps, key) => {
    if (parentIds.has(key)) return;
    groups.push({ id: key, parent: null, steps: steps.sort((a, b) => b.workflowOffset - a.workflowOffset) });
  });

  return groups
    .filter((group) => [group.parent, ...group.steps].filter(Boolean).some(taskMatchesCurrentView))
    .sort((a, b) => {
      const aTask = a.parent || a.steps[0];
      const bTask = b.parent || b.steps[0];
      if (aTask.completed !== bTask.completed) return Number(aTask.completed) - Number(bTask.completed);
      return parseLocalDate(aTask.dueDate) - parseLocalDate(bTask.dueDate);
    });
}
    ...
    <article class="task-row flex h-full min-w-0 flex-col rounded-2xl border p-4 shadow-sm transition ${isParent ? "bg-blue-50/70 ring-4 ring-blue-50" : "bg-white"} ${borderStyle} ${task.completed ? "task-complete" : ""}" data-task-id="${escapeHtml(task.id)}">
      ...
      <div class="mt-4 flex flex-1 items-start gap-3">
        ...
        <div class="min-w-0 flex-1">
          ...
          ${task.memo && isParent ? `<p class="mt-2 line-clamp-2 text-xs leading-5 text-slate-500">${escapeHtml(task.memo)}</p>` : ""}
        </div>
      </div>
      <div class="mt-4 flex items-center justify-between gap-3 border-t border-slate-100 pt-3 text-[11px] font-medium text-slate-500">
        <span><i class="fa-regular fa-calendar mr-1.5 text-slate-400" aria-hidden="true"></i>${isParent ? "최종 마감" : "단계 마감"}: <strong class="font-semibold text-slate-600">${escapeHtml(task.dueDate)}</strong></span>
...
function render() {
  ...
  const visibleStepCount = groups.reduce((count, group) => count + group.steps.length, 0);
  elements.resultCount.textContent = `업무 ${groups.length}건${visibleStepCount ? ` · 준비 단계 ${visibleStepCount}건` : ""}`;
  elements.taskList.innerHTML = groups.map(workflowGroupTemplate).join("");
...
async function handleDelete(button) {
  ...
  const task = state.tasks.find((item) => item.id === row.dataset.taskId);
  if (!task) return;
  const linkedSteps = task.autoGenerated ? [] : state.tasks.filter((item) => item.parentTaskId === task.id);
  const targets = [task, ...linkedSteps];
  const message = linkedSteps.length
    ? `이 업무와 연결된 준비 단계 ${linkedSteps.length}개를 모두 삭제하시겠습니까?`
    : "이 업무를 삭제하시겠습니까?";
  if (!window.confirm(message)) return;

  const previousTasks = [...state.tasks];
  const targetIds = new Set(targets.map((item) => item.id));
  state.tasks = state.tasks.filter((item) => !targetIds.has(item.id));
  render();
  try {
    for (const target of targets) await taskService.deleteTask(target.id);
    showToast(linkedSteps.length ? "업무와 연결된 준비 단계를 삭제했습니다." : "업무를 삭제했습니다.");
  } catch (error) {
